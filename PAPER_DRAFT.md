...
**(F2) Compute budget 통제.** 모든 representation 은 동일 max iteration (30,000) 으로 학습하며, default hyperparameter [8], [9] 외 추가 튜닝은 적용하지 않는다. **Resolution 통제:** 9 configurations × 2 domains 의 representation stage 입력 frame 은 모두 `--resolution 2` (1920×1080 → 960×540 다운샘플) 로 균일 적용된다. 본 다운샘플은 단일 RTX 4090 (24 GiB) GPU 의 image 적재 한계 (full-resolution 시 ~22 GiB 발생; §5.5 L9 참조) 를 회피하면서 모든 configuration 에 동일하게 적용되어 inter-configuration fairness 를 보존한다. PSNR / SSIM 의 절대값은 native resolution 대비 낮아지나 (§5.5 L9), *configuration 간 ranking* 은 보존됨이 본 fairness 의 핵심 commitment.
...
**(L9) Resolution downsample 의 PSNR ceiling 영향.** §3.6 F2 의 GPU 메모리 제약 (단일 RTX 4090 24 GiB) 에 의해 9 configurations × 6 sites 의 representation stage 가 `--resolution 2` (1920×1080 → 960×540) 로 균일 다운샘플되었다. 본 다운샘플은 native resolution 대비 PSNR 절대값을 ~1-2 dB 낮추며 SSIM/LPIPS 의 *absolute reference scale* 에 영향을 준다. *Inter-configuration ranking* 은 모든 configurations 가 동일 resolution 에서 평가되므로 보존되나, 본 논문의 PSNR/SSIM 절대값을 다른 paper 의 native-resolution 결과와 *직접 비교* 시 본 downsample 을 고려해야 한다. 본 한계는 §3.6 F2 의 fairness commitment 와 *상충 없이 명시* 되어 inter-configuration verdict (§5.1 Tab. 4) 의 외부타당도를 보존한다 — full-resolution 평가는 multi-GPU (A100 80 GiB 이상) 환경의 future work 으로 deferred (§6.2 F3 on-device deployment 의 sub-slot).
