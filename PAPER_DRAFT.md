...
전처리 모듈은 9 configurations에 동일하게 적용되어 공정성을 보장하는 모든 입력 측의 전처리를 묶은 단위이며, 본 논문의 시스템 기여(C2-sub-a)의 α-contribution에 해당한다. 모듈은 세 sub-stage로 구성된다.

**(a) FPS filtering.** 보디캠 원본 30 fps에서 사용자 사양 5–10 fps로 균일 downsample. 본 논문 default는 5 fps로 고정하며, frame budget 정규화(§3.6) 시 두 도메인에 동일 FPS 적용.

**(b) Sharpness threshold.** Laplacian variance 기반 blur 검출 [15]. Pertuz et al. [15] 의 권장 normalized threshold를 inter-frame 분포의 30 percentile로 보정하여 채택. Threshold 미달 frame은 sample set에서 제외하되 timestamp는 보존하여 §4.4 block bootstrap 의 block boundary 정렬에 사용.

**(c) Masking automation.** 작업자·이동 자재 등 dynamic instance는 SAM2 [13] 의 person/object class mask 또는 YOLOv8-seg [14] 의 instance segmentation으로 자동 생성. 두 도구 모두 산업현장 fine-tuning 없이 default weight를 사용하며, 마스크 IoU의 도구 간 일치도는 §4.5e에서 sensitivity로 보고. Mask는 §4.5e의 cluster B.1 ablation에서 on/off 두 가지 condition으로 사용된다.

[Alg. 1: Preprocessing pseudocode — input video → (FPS filter) → (Laplacian sharpness threshold) → (SAM2 ∨ YOLO instance mask) → (filtered frame set + per-frame instance mask)].

본 모듈의 output (filtered frames + masks)은 9 configurations의 Stage 3 입력으로 공통 진입한다. Configuration 간 차이는 Stage 3·4의 atomic method 조합에서만 발생한다.
...
**(F4) 영상 길이 정규화.** Industrial 원본(약 5 min/site)과 Library 원본(약 1–2 min/site)의 비대칭을 제거하기 위해 두 도메인 모두 동일 frame budget N_frames = 1,500 (= 5 fps × 5 min) 으로 trim/sub-sample한다. Library 원본이 짧을 경우 zero-pad 대신 *동일 frame budget 내 dense temporal sampling* 으로 채우며, industrial 원본이 길 경우 등간격 sub-sample을 사용한다. 본 정규화는 Δ 비교에서 영상 길이가 third variable로 작용하는 것을 차단한다.
...
**4.5e Dynamic Object Removal Ablation — Cluster B.1.** SAM2 [13] 또는 YOLOv8-seg [14] 로 생성된 person/object instance mask 의 on/off ablation. **Mask on** (dynamic instance pixel을 학습 frame 에서 제외) / **mask off** (dynamic instance pixel을 그대로 학습에 포함) 의 두 condition 에 대해 동일 site × method 의 reconstruction 품질과 pose ATE drift 를 비교한다. DynaSLAM [23] 류의 dynamic-aware pipeline 부재가 pose estimation drift에 미치는 영향을 mask off condition 에서 측정. [Fig. 6: Trajectory drift (ATE) vs dynamic object pixel fraction, P1 vs P9]. *비고:* Cluster B.2 (scale 변동) 는 frame-level mask 정의가 depth prior 를 추가로 요구하여 본 ablation 행렬에서 inviable 하므로 §5.5 limitations 의 future-work slot 으로 미룬다 — 본 논문은 B.2 에 대해서는 cluster prediction 만 §3.5 에 명시하고 ablation 검증은 수행하지 않는다.
...
**Internal validity.** §4.5 ablation 의 mask 생성 알고리즘은 ground-truth mask 가 없으므로 *cross-method consistency* (SAM2 [13] vs YOLOv8-seg [14] mask IoU) 와 *cross-sub-cluster IoU* (§4.5d-aux Tab. 5) 로 부분적으로 보완된다. 그러나 mask 자체의 noise 가 ablation effect 의 attenuation bias 로 작용할 가능성은 남는다.
