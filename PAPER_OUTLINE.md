...
| 2026-05-13 (sharp JPG → PNG 정규화 step 추가) | 사용자 video-to-image 모듈이 sharp = JPG, mask = PNG 두 포맷 혼합 출력 확인 → mask 는 PNG 유지 (binary mask lossless 필수), sharp 는 PNG 통일 결정. (1) **RUN_PILOT.md §3.3 신규** — JPG → PNG 일괄 변환 ad-hoc (ImageMagick mogrify option A, Python PIL option B) + 검증 명령 + 비용 추정 (~5-10 min wall-time, ~2-3× 디스크 증가). 후속 §3.3 → §3.4 (symlink) / §3.4 → §3.5 (mask 정책) / §3.5 → §3.6 (frame budget) 으로 한 칸 밀음. (2) **data/README.md §5** 의 PNG-only 정책 기존 명시 유지 — sharp 도 본 정책에 정렬됨을 §3.3 ad-hoc 으로 보장. 본 결정의 fairness 근거는 §3.6 F3 (전처리 통일) + PSNR/SSIM noise floor JPEG-artifact 제거 + §4.5d cluster A.3 오염 vs JPEG block artifact 혼동 차단. | experiments/RUN_PILOT.md §3.3 신규 |
