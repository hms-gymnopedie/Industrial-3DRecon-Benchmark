...
| 2026-05-18 (GPU 활용 정책 전환 — resolution=2 + cuda) | 이전 commit (data_device=cpu) 의 wall-time +20-30% 부담 회피 + GPU 적극 활용을 위해 정책 전환. **run_pipeline_p1.sh / p9.sh** 의 train docker call 을 `--resolution 2` (1920×1080 → 960×540) + `--data_device cuda` (default 복원) 으로 변경. GPU 사용량 ~5-6 GiB (image 1/4) + 학습 ~15-20 GiB, 단일 4090 안전. **PAPER §3.6 F2** wording 갱신 — Resolution 통제 commitment 추가 (9 configurations 균일 적용 → fairness 보존, PSNR 절대값은 native 대비 -1~2 dB). **§5.5 L9 신규** — resolution downsample 의 PSNR ceiling 영향 + multi-GPU full-res 평가 future work (§6.2 F3 sub-slot). **RUN_PILOT.md §11.4** OOM troubleshooting 항목 wording 갱신 (cpu policy → cuda+res2 policy). | experiments/scripts/run_pipeline_p{1,9}.sh · PAPER_DRAFT §3.6 F2 · §5.5 L9 · RUN_PILOT.md §11.4 |
| 2026-05-18 (ENTRYPOINT 충돌 fix — `--entrypoint ""`) | Pilot 실행 중 m8/m9 image 의 `--user` + nvidia_entrypoint.sh ENTRYPOINT 가 충돌 — default CMD ("3DGS rasterizer + simple_knn OK") 가 user 명령 대신 실행되고 그 후 train.py 호출이 path missing 으로 fail. 모든 8 docker run (P1 의 3개 + P9 의 5개) 에 `--entrypoint ""` 추가하여 image ENTRYPOINT 무력화 → user 명령 직접 실행. 추가로 사용자 측 운용 단순화 차원에서 P1 Stage 1 의 COLMAP `--camera_model OPENCV` → `PINHOLE` 변경 (distortion 0 가정 — image_undistorter trivial 이지만 sparse/0 재배치는 그대로 유지). **RUN_PILOT.md §11.5 신규** ENTRYPOINT troubleshooting + 검증 우회 명령. | experiments/scripts/run_pipeline_p{1,9}.sh · RUN_PILOT.md §11.5 |
| 2026-05-19 (entrypoint binary 직접 지정 + metrics.py call + intrinsics image) | docker 가 `--entrypoint ""` 를 "no entrypoint and no command" 로 reject → 8 docker run 마다 정확한 binary (colmap or python) 직접 지정 + args 의 first token 제거. P1 Stage 2 정상 학습 완료 (I-1, 14 min) 후 두 추가 issue 발견 — (i) PSNR/SSIM/LPIPS 가 metrics.json 에 없음 (3DGS train.py 는 학습만, evaluation 은 metrics.py 별도 호출 필요), (ii) intrinsics 추출 명령이 ars/m1_colmap (python 미포함) 에서 fail. 영구 fix: **run_pipeline_p1.sh** 에 Stage 2.5 (M8 metrics.py call → results.json 생성, idempotent) 추가 / **run_pipeline_p9.sh** 에 Stage 3.5 (M9 metrics.py) 추가 / **RUN_PILOT.md §4·§7** 의 intrinsics 추출 명령에서 ars/m1_colmap → ars/m8_3dgs (python+numpy 보유). compute_metrics.py 가 results.json 을 읽어 PSNR/SSIM/LPIPS 자동 fill. | experiments/scripts/run_pipeline_p{1,9}.sh · RUN_PILOT.md §4·§7 |
| 2026-05-19 (lietorch ABI fix — source build) | P9 Stage 1 (M7 MASt3R-SLAM) 진입 시 `OSError: lietorch/lib/liblietorch.so: undefined symbol: _ZNK3c106SymInt13toSymNodeImplEv` — pip prebuilt `lietorch` wheel 이 본 image 의 PyTorch 2.1.0+cu118 과 다른 PyTorch 버전으로 build 되어 C++ ABI mismatch (2 host 에서 동일 확인 → 모든 host 의 m7 image broken). **m7_mast3r_slam/Dockerfile** 의 `pip install lietorch` (wheel) → `pip install --no-build-isolation git+https://github.com/princeton-vl/lietorch.git` (source build) 변경 → 본 image 의 PyTorch ABI 와 일치하는 binary 생성. image rebuild 필요 (~15-25 min per host) 또는 1 host build → save/load distribute. **RUN_PILOT.md §11.6 신규** lietorch ABI troubleshooting + rebuild + distribute 명령. | experiments/docker/m7_mast3r_slam/Dockerfile · RUN_PILOT.md §11.6 |
| 2026-05-19 (PYTHONPATH thirdparty/mast3r 수정) | lietorch fix 후 P9 Stage 1 재시도 시 `ModuleNotFoundError: No module named 'mast3r'` — MASt3R-SLAM repo 의 mast3r submodule 이 `/opt/mast3r_slam/thirdparty/mast3r/` 에 nested 되어 있는데 Dockerfile 의 PYTHONPATH 는 `/opt/mast3r_slam/mast3r` (존재 안 함) 를 가리킴. **run_pipeline_p9.sh** Stage 1 + Stage 2 docker run 에 `-e PYTHONPATH=/opt/mast3r_slam:/opt/mast3r_slam/thirdparty/mast3r:.../dust3r:.../in3d` 추가 → rebuild 없이 즉시 작동 (사용자 측 부담 최소). **m7 Dockerfile ENV PYTHONPATH** 도 동일 수정 (next rebuild 시 redundant 정합). **RUN_PILOT.md §11.7 신규** ModuleNotFoundError mast3r troubleshooting + ad-hoc 검증 명령. | experiments/scripts/run_pipeline_p9.sh · docker/m7_mast3r_slam/Dockerfile · RUN_PILOT.md §11.7 |
| 2026-05-19 (faiss-cpu 추가 — m7) | PYTHONPATH fix 후 P9 Stage 1 진입 시 `ModuleNotFoundError: No module named 'faiss'` — MASt3R 의 retrieval 모듈이 Facebook AI 의 vector similarity library 사용. m7 Dockerfile 의 pip install 목록에 `faiss-cpu` 추가. 사용자 측 rebuild 부담 회피를 위해 **ad-hoc docker commit** 절차 (임시 container → pip install faiss-cpu → docker commit → host-local image 즉시 갱신; 5분 안) 도 RUN_PILOT.md §11.8 에 명시. | docker/m7_mast3r_slam/Dockerfile · RUN_PILOT.md §11.8 |
| 2026-05-19 (m7 dependency 일괄 재정비) | faiss-cpu fix 후 또 `ModuleNotFoundError: No module named 'asmk.index'` — m7 Dockerfile 의 `pip install -e . \|\| echo "[warn]"` 가 silent skip 으로 mast3r-slam setup.py 의 dependency chain 전체를 누락. 일괄 추가 (whack-a-mole 차단): **asmk** (git+source, cython build) + **kapture**, **kapture-localization**, **timm** (mast3r/retrieval) + **pypose**, **kornia**, **omegaconf**, **pyflann-py3** (general SLAM/CV utils). `pip install -e .` 의 silent fail (`\|\| echo`) 제거 — build 실패 시 명시적 error. RUN_PILOT.md §11.8 의 ad-hoc 명령도 일괄 install 로 갱신 + 최종 import 검증 추가 (`mast3r_slam.global_opt` import 확인). | docker/m7_mast3r_slam/Dockerfile · RUN_PILOT.md §11.8 |
| 2026-05-19 (asmk Cython source build 정정) | 이전 commit 의 `pip install git+https://github.com/jenicek/asmk.git` 가 .pyx 컴파일 결과를 site-packages 에 올리지 못해 또 fail. **naver/mast3r 공식 install 절차** 그대로: (1) `pip install cython`, (2) `git clone https://github.com/jenicek/asmk.git /opt/asmk`, (3) `cd /opt/asmk/cython && python setup.py build_ext --inplace`, (4) `/opt/asmk` 를 ENV PYTHONPATH 에 추가. **run_pipeline_p9.sh** 의 두 -e PYTHONPATH 에 `/opt/asmk` 추가. **RUN_PILOT.md §11.8** ad-hoc 명령 정확한 절차로 재작성 + 단계별 검증 (`asmk.index` import → `global_opt` import). | docker/m7_mast3r_slam/Dockerfile · run_pipeline_p9.sh · RUN_PILOT.md §11.8 |
