...
### 11.3 `FileNotFoundError: pose_undistorted/sparse/0/images.txt` + permission denied

증상 1: P1 Stage 2 (또는 P9 Stage 3) 에서 `FileNotFoundError: pose_undistorted/sparse/0/images.txt` 발생.

증상 2: ad-hoc `mkdir -p sparse/0` 시 `Permission denied`.

원인:
- COLMAP `image_undistorter` 가 sparse model 을 `sparse/` 에 직접 출력 (`sparse/0/` 아님) → 3DGS/2DGS dataset_readers 의 `sparse/0/` 기대와 mismatch
- Docker container 가 root 권한으로 산출물 생성 → host 의 일반 user 가 mkdir/mv 불가

해결 — sudo 없는 학교 서버에서도 가능. **Docker container 내부에서 root 권한으로 정리** (docker daemon 자체가 root 라 sudo 우회):
...
docker run --rm \
    ...
    ars/m1_colmap:3.9.1 \
    bash -c '
        DIR=/data/outputs/P1/I-1/run-01/pose_undistorted
        if [ -f "$DIR/sparse/cameras.bin" ] && [ ! -d "$DIR/sparse/0" ]; then
            mkdir -p "$DIR/sparse/0"
            mv "$DIR/sparse"/*.bin "$DIR/sparse/0/"
        fi
        chown -R '"$(id -u):$(id -g)"' "$DIR"
    '
```

대량 정리 (outputs/ 전체 root 소유 정리):
```bash
docker run --rm \
    -v /data/minsuh/experiment/data:/data \
    ars/m1_colmap:3.9.1 \
    chown -R $(id -u):$(id -g) /data/outputs
```

영구 fix — `git pull` 후 다음 run 부터 자동 적용:
